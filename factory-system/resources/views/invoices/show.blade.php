...
<x-page-header :title="$invoice->invoice_number" :description="$invoice->customer?->name" :back="route('invoices.index')">
    @can('issue', $invoice)
        <form method="POST" action="{{ route('invoices.issue', $invoice) }}">@csrf<x-btn type="submit">{{ __('invoices.issue') }}</x-btn></form>
    @endcan
    <x-btn :href="route('invoices.print', $invoice)" variant="ghost">{{ __('ui.actions.print') }}</x-btn>
    <x-btn :href="route('invoices.download', $invoice)" variant="secondary">{{ __('invoices.download') }}</x-btn>
</x-page-header>
...
@can('void', $invoice)
    <x-card :title="__('invoices.void')" class="mt-6">
        <form method="POST" action="{{ route('invoices.void', $invoice) }}" class="grid gap-3 md:grid-cols-[1fr_auto]">
            @csrf
            <x-form-input name="reason" :label="__('invoices.void_reason')" :value="__('invoices.voided_manually')" required />
            <div class="flex items-end"><x-btn type="submit" variant="danger">{{ __('invoices.void') }}</x-btn></div>
        </form>
    </x-card>
@endcan
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 py-3 last:border-0">
            ...
            @can('deletePayment', $invoice)
                <form method="POST" action="{{ route('invoices.payments.destroy', [$invoice, $payment]) }}">
                    @csrf
                    @method('DELETE')
                    <x-btn type="submit" size="sm" variant="danger">{{ __('ui.actions.delete') }}</x-btn>
                </form>
            @endcan
        </div>
    @empty
