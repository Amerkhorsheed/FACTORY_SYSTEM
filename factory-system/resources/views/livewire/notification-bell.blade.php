<div class="relative" x-data="{ open: false }">
    ...
    <div x-show="open" 
        ...
        <div class="flex items-center justify-between px-4 py-3 bg-ink-50 border-b border-ink-200">
            <h3 class="font-bold text-ink-900 font-cairo">الإشعارات</h3>
            @if($unreadCount > 0)
            ...
            @forelse($notifications as $notification)
                <div class="flex items-start gap-3 p-4 border-b border-ink-100 hover:bg-ink-50 transition {{ $notification->read_at ? 'opacity-60' : 'bg-brand-50/30' }}">
                    ...
                    @if(!$notification->read_at)
                        <button wire:click="markAsRead('{{ $notification->id }}')" class="flex-shrink-0 text-brand-600 hover:text-brand-800" title="تحديد كمقروء">
        ...
        <!-- Footer -->
        <div class="p-2 bg-ink-50 border-t border-ink-200 text-center">
            <a href="#" class="text-sm text-brand-600 hover:text-brand-800 font-bold transition">
                عرض كل الإشعارات
            </a>
        </div>
    </div>
